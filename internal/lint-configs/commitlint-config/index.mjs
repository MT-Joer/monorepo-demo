/*
  ...
  目的：配置 commitlint（cz-git）以强制仓库的 commit message 规范。
  关键点：
  - 使用 conventional commits 作为基础以支持自动化 changelog/release
  - 通过插件和脚本动态生成 scope 白名单（基于 monorepo packages）
  - 为交互式提交提示提供别名/默认值以降低误提交成本

  注意：该文件通过读取 git 状态来尝试预填充 scope（提高体验）。
*/
