<template>
    <div :class="cn('bg-card h-full rounded-md', className)">
        <VxeGrid ref="gridRef"
            ...
            <!-- 继承默认的slot -->
            <template v-for="slotName in delegatedSlots"
                      :key="slotName"
                      #[slotName]="slotProps">
            ...
            <template #toolbar-tools="slotProps">
                ...
                <VxeButton v-if="gridOptions?.toolbarConfig?.search && !!formOptions"
                           circle
                           class="ml-2"
            ...
            <template #form>
                <div v-if="formOptions"
                    ...
                    <slot name="form">
                        <Form>
                            <template v-for="slotName in delegatedFormSlots"
                                      :key="slotName"
                                      #[slotName]="slotProps">
                                <slot :name="`${FORM_SLOT_PREFIX}${slotName}`"
                                      v-bind="slotProps">
                                </slot>
                            </template>
