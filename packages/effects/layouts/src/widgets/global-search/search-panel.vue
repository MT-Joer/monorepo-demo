    ...
    <VbenScrollbar>
        <div class="!flex h-full justify-center px-2 sm:max-h-[450px]">
            ...
            <ul v-show="searchResults.length > 0" class="w-full">
                <li v-if="searchHistory.length > 0 && !keyword"
                    class="text-muted-foreground mb-2 text-xs">
                    {{ $t('ui.widgets.search.recent') }}
                </li>
                <li v-for="(item, index) in uniqueByField(searchResults, 'path')"
                    :key="item.path"
                    ...
                    <VbenIcon class="mr-2 size-5 flex-shrink-0"
                              fallback
                              :icon="item.icon" />

                    <span class="flex-1">{{ item.name }}</span>
                    <div class="flex-center dark:hover:bg-accent hover:text-primary-foreground rounded-full p-1 hover:scale-110"
