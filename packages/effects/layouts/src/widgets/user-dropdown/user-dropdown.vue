<template>
    <LockModal v-if="preferences.widget.lockScreen"
               :avatar="avatar"
               :text="text"
               @submit="handleSubmitLock" />

    <LogoutModal :cancel-text="$t('common.cancel')"
                 centered
    ...
    <DropdownMenu v-model:open="openPopover">
        ...
        <DropdownMenuContent class="mr-2 min-w-[240px] p-0 pb-1">
            <div ref="refContent">
                ...
                <DropdownMenuItem v-for="menu in menus"
                                  :key="menu.text"
                                  class="mx-1 flex cursor-pointer items-center rounded-sm py-1 leading-8"
                                  @click="menu.handler">
                    <VbenIcon class="mr-2 size-4" :icon="menu.icon" />
                    {{ menu.text }}
                </DropdownMenuItem>
