...
<template>
    <view class="mt-6">
        ...
        <button class="my-4" @click="getFoo">
            测试 GET 请求
        </button>
        <view class="text-xl">
            请求数据如下
        </view>
        <view class="text-green h-10">
            {{ JSON.stringify(data) }}
        </view>
        <view class="text-xl">
            完整数据
        </view>
        <view class="text-green h-20">
            {{ JSON.stringify(originalData) }}
        </view>
        <button class="my-4" @click="postFoo">
            测试 POST 请求
        </button>
        <view class="text-xl">
            请求数据如下
...
import { getFooAPI, IFooItem, postFooAPI } from "@/service/foo";
