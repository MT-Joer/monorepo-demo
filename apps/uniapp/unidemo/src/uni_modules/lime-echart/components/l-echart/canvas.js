...
export class Canvas {
    ...
    _initStyle(ctx, child) {
        ...
        const colorReg = /#([0-9a-f])([0-9a-f])([0-9a-f])\b/gi;
                ...
                set: value => {
                    ...
                    if (style !== "fillStyle" && style !== "strokeStyle" || value !== "none" && value !== null) {
                        ...
                        if (typeof value === "object") {
                            if (value.hasOwn("colorStop") || value.hasOwn("colors")) {
                        ...
                        if (colorReg.test(value)) {
                            value = value.replaceAll(colorReg, "#$1$1$2$2$3$3");
                        }
                        // #endif
                        ctx[`set${  style.charAt(0).toUpperCase()  }${style.slice(1)}`](value);
                    }
        ...
        if (!ctx.measureText || getDeviceInfo().osName == "harmonyos") {
            ...
            ctx.measureText = (text, font) => {
                ...
                if (font) {
                    fontSize = Number.parseInt(font.match(/([\d.]+)px/)[1]);
                }
                fontSize /= 2;
...
export function setCanvasCreator(echarts, { canvas, node }) {
    ...
    function loadImage(src, onload, onerror) {
        ...
        if (node && node.createImage) {
            ...
            img.addEventListener("load", onload.bind(img));
