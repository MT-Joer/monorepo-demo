<template>
    <view v-if="canvasId"
        ...
        <canvas v-if="isOffscreenCanvas"
                :canvas-id="offscreenCanvasId"
                :style="offscreenStyle">
        </canvas>
        <!-- #endif -->
        <!-- #ifdef APP-NVUE -->
        <web-view :id="canvasId"
                  ref="webview"
                  class="lime-echart__canvas"
...
export default {
    ...
    methods: {
        ...
        onMessage(e) {
            ...
            if (event) {
                this.chart.dispatchAction(event.replaceAll('"', ""), options);
            }
            if (file) {
                thie.file = file;
            }
        ...
        async init(echarts, ...args) {
            ...
            // #endif
            let callback; let opts={}; let theme=null;
            ...
            return getRect(`#${this.canvasId}`, { context: this, type: this.use2dCanvas ? "fields": "boundingClientRect" }).then(res => {
                if (res) {
                    ...
                    if (node) {
                        ...
                        // #ifdef MP-TOUTIAO
                        dpr = this.isPC ? 1 : devicePixelRatio;// 1.25
                        // #endif
        ...
        touchEnd(e) {
            ...
            if (this.chart) {
                ...
                const handler = this.chart.getZr().handler;
                const isClick = Math.abs(touch.x - this.startX) < 10 && Date.now() - this.startT < 200;
                ...
                if (isClick) {
                    ...
                    this.endTimer = setTimeout(() => {
                        dispatch.call(handler, "mousemove", { x: 999_999_999, y: 999_999_999 });
                        dispatch.call(handler, "mouseup", { x: 999_999_999, y: 999_999_999 });
                    }, 50);
                }
            }
        },
