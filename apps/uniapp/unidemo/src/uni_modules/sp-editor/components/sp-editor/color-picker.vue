<template>
    <view v-if="show"
        ...
        <view class="t-box" :class="{ active }">
            <view class="t-header">
                <view class="t-header-button" @click="close">
                    取消
                </view>
                <view class="t-header-button" @click="confirm">
                    确认
                </view>
            </view>
...
export default {
     ...
     */
        setPosition(x, y, index) {
            ...
            this.site[index].left = Math.max(0, Math.min(Number.parseInt(x - left), width));
            if (index == 0) {
                this.site[index].top = Math.max(0, Math.min(Number.parseInt(y - top), height));
                // 设置颜色
                this.hsb.s = Number.parseInt((100 * this.site[index].left) / width);
                this.hsb.b = Number.parseInt(100 - (100 * this.site[index].top) / height);
                this.setColor();
                this.setValue(this.rgba);
            } else {
     ...
     */
        ...
        setControl(index, x) {
            ...
            if (index == 1) {
                this.hsb.h = Number.parseInt((360 * x) / width);
     ...
     */
        ...
        setColorBySelect(getrgb) {
            ...
            let rgb = {};
            rgb = {
                r: r ? Number.parseInt(r) : 0,
                g: g ? Number.parseInt(g) : 0,
                b: b ? Number.parseInt(b) : 0,
                a: a || 0
            };
            this.rgba = rgb;
        ...
        changeViewByHsb() {
            ...
            this.site[0].left = Number.parseInt((this.hsb.s * a.width) / 100);
            this.site[0].top = Number.parseInt(((100 - this.hsb.b) * a.height) / 100);
...
.t-transparency {
  width: 100%;
  height: 100%;
  background: linear-gradient(to right, rgb(0 0 0 / 0%) 0%, rgb(0 0 0));
}
