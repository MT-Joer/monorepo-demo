...
<template>
    <view :id="`ChartBoxId${ cid}`" class="chartsview">
        <view v-if="mixinDatacomLoading">
            <!-- 自定义加载状态，请改这里 -->
            <qiun-loading :loading-type="loadingType" />
        </view>
        <view v-if="mixinDatacomErrorMessage && errorShow" @tap="reloading">
            <!-- 自定义错误提示，请改这里 -->
            <qiun-error :error-message="errorMessage" />
        </view>
        <!-- APP和H5采用renderjs渲染图表 -->
        <!-- #ifdef APP-VUE || H5 -->
        <block v-if="echarts">
            <view v-show="showchart"
                  :id="`EC${ cid}`"
        ...
        <block v-else>
            <view :id="`UC${ cid}`"
                ...
                <canvas v-show="showchart"
                        :id="cid"
                        :canvasId="cid"
                        :disable-scroll="disableScroll"
                        :style="{ width: `${cWidth }px`, height: `${cHeight }px`, background }"
                        @error="_error" />
            </view>
        </block>
        <!-- #endif -->
...
function formatterAssign(args, formatter) {
    for (const key in args) {
        if (args.hasOwn(key) && args[key] !== null && typeof args[key] === "object") {
...
function debounce(fn, wait) {
    ...
    return function () {
        ...
        timer = setTimeout(() => {
            timer = false;
            Reflect.apply(fn, this, arguments); // 把参数传进去
        }, wait);
    };
}
...
export default {
    ...
    computed: {
        ...
        eoptsProps() {
            return structuredClone(this.eopts);
        },
        chartDataProps() {
            return structuredClone(this.chartData);
        },
    },
    watch: {
        chartDataProps: {
    ...
    methods: {
        ...
        localdataInit(resdata) {
            ...
            if (this.groupEnum.length > 0) {
                for (const resdatum of resdata) {
                    for (let j = 0; j < this.groupEnum.length; j++) {
                        if (resdatum.group === this.groupEnum[j].value) {
                            resdatum.group = this.groupEnum[j].text;
                        }
                    }
                }
            }
            if (this.textEnum.length > 0) {
                for (const resdatum of resdata) {
                    for (let j = 0; j < this.textEnum.length; j++) {
                        if (resdatum.text === this.textEnum[j].value) {
                            resdatum.text = this.textEnum[j].text;
                        }
                    }
                }
            }
            ...
            if (tmpseries.length === 0) {
                ...
                if (needCategories === true) {
                    for (const tmpcategory of tmpcategories) {
                        let seriesdata = 0;
                        for (const resdatum of resdata) {
                            if (resdatum.text === tmpcategory) {
                                seriesdata = resdatum.value;
                            }
                        }
                    ...
                    for (const resdatum of resdata) {
                        tmpseries[0].data.push({ name: resdatum.text, value: resdatum.value });
                    }
                }
                // 如果有分组名
            } else {
                for (const tmpsery of tmpseries) {
                    // 如果有categories
                    if (tmpcategories.length > 0) {
                        for (const tmpcategory of tmpcategories) {
                            let seriesdata = 0;
                            for (const resdatum of resdata) {
                                if (tmpsery.name === resdatum.group && resdatum.text === tmpcategory) {
                                    seriesdata = resdatum.value;
                                }
                            }
                            tmpsery.data.push(seriesdata);
                        }
                        // 如果传了group而没有传text，即没有categories（正常情况下这种数据是不符合数据要求规范的）
                    } else {
                        for (const resdatum of resdata) {
                            if (tmpsery.name === resdatum.group) {
                                tmpsery.data.push(resdatum.value);
                            }
                        }
                    }
                }
        ...
        resizeHandler() {
            ...
            // #endif
                ...
                .boundingClientRect((data) => {
                    this.showchart = true;
                    if (data.width > 0 && data.height > 0 && (data.width !== this.cWidth || data.height !== this.cHeight)) {
                        this.checkData(this.drawData);
                    }
                })
                .exec();
        ...
        getCloudData() {
            ...
            this.mixinDatacomGet()
                ...
                .catch((error) => {
                    this.mixinDatacomLoading = false;
                    this.showchart = false;
                    this.mixinDatacomErrorMessage = `请求错误：${error}`;
                });
        },
...
export default {
  ...
  methods: {
    ...
    ecinit(newVal, oldVal, owner, instance){
      ...
      let eopts = structuredClone(newVal)
      let type = eopts.type;
      //载入并覆盖默认配置
    ...
    ucinit(newVal, oldVal, owner, instance){
      ...
      let cid = structuredClone(newVal.canvasId)
      this.rid = cid
      that[cid] = this.$ownerInstance
      cfu.option[cid] = structuredClone(newVal)
