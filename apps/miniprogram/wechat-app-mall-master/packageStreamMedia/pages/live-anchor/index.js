...
Page({
   ...
   */
    ...
    setScrollTop() {
        ...
        wx.createSelectorQuery().in(e).select(".barrage").boundingClientRect((res) => {
            console.log(res);
            e.setData({
        ...
        query.in(e).select(".item-outer").boundingClientRect((res) => {
            if (res.bottom > e.data.chatbottom) {
                const temp = Math.ceil(Number.parseInt(res.bottom) - Number.parseInt(e.data.chatbottom));
                e.setData({
   ...
   */
    ...
    // options.id = 13
        ...
        const query = wx.createSelectorQuery();
        query.select(".barrage").boundingClientRect((rect) => {
            // console.log(rect)
        }).exec();
    },
    // 主推商品详情
    async toDetail(e) {
        ...
        switch (res.data.basicInfo.supplyType) {
            case "cps_jd": {
                wx.navigateTo({
   ...
   */
    ...
    handleInteractionTap() {
        ...
        const temp = data.showInput ? app.globalData.screenH : data.fullScreenHeight - 50;
        this.setData({
