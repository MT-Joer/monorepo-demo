...
class UserController {
    ...
    // console.log(TAG_NAME, 'addUser', data)
        ...
        if (Array.isArray(incomingUserList) && incomingUserList.length > 0) {
            incomingUserList.forEach((item) => {
                ...
                this._emitter.emit(TRTC_EVENT.REMOTE_USER_JOIN, { userID, userList: this.userList });
   ...
   */
    ...
    // console.log(TAG_NAME, 'removeUser', data)
        ...
        if (Array.isArray(incomingUserList) && incomingUserList.length > 0) {
            incomingUserList.forEach((item) => {
                ...
                this._emitter.emit(TRTC_EVENT.REMOTE_USER_LEAVE, { userID, userList: this.userList, streamList: this.streamList });
                user = undefined;
    ...
    /**
   * 重置所有user 和 steam
   * @returns {object} Object containing reset userList and streamList
   */
    reset() {
        ...
        if (Array.isArray(incomingUserList) && incomingUserList.length > 0) {
            incomingUserList.forEach((item) => {
                ...
                if (user) {
                    ...
                    this._emitter.emit(eventName, { stream, streamList: this.streamList, userList: this.userList });
    ...
    userEventHandler(event) {
        ...
        if (event.detail.message && typeof event.detail.message === "string") {
            try {
                ...
                console.warn(TAG_NAME, "userEventHandler 数据格式错误", error);
                return false;
            }
        } else {
