...
Component({
        ...
        _initEventEmitter() {
            ...
            this.userController.on(TRTC_EVENT.REMOTE_USER_JOIN, (event)=>{
                ...
                this.setData({
                    ...
                    if (this.data._historyUserList.includes(event.data.userID)) {
     ...
     * @param {number} params.type 0-为之， 1-语音通话，2-视频通话
     */
        call({ userID, type }) {
            // 生成房间号，拼接URL地址
            const roomID = Math.floor(Math.random() * 100_000_000 + 1); // 随机生成房间号
     ...
     *
     * @param {object} params - 邀请参数
     * @param {string[]} params.userIDList 邀请列表
        ...
        groupCall(params) {
            this.tsignaling.inviteInGroup({
                ...
                data: JSON.stringify({
                    ...
                    room_id: Math.floor(Math.random() * 100_000_000 + 1),
                }),
            }).then((res) => {
        ...
        _getPushUrl(roomId) {
            ...
            if (ENV.IS_TRTC) {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    let roomID = "";
                    roomID = /^\d+$/.test(roomId) ?   `&roomid=${  roomId}` : `&strroomid=${  roomId}`;
                    setTimeout(()=> {
