...
Page({
    ...
    onLoad(e) {
        ...
        if (e && e.scene) {
            ...
            if (scene) {
                wx.setStorageSync("referrer", scene.slice(11));
            }
        }
    ...
    // https://www.yuque.com/apifm/nu0f75/wg5t98
        ...
        if (res.code === 0) {
            res.data.result.forEach(ele => {
                const _now = Date.now();
                if (ele.dateStart) {
                    ele.dateStartInt = new Date(ele.dateStart.replaceAll("-", "/")).getTime() - _now;
                }
                if (ele.dateEnd) {
                    ele.dateEndInt = new Date(ele.dateEnd.replaceAll("-", "/")).getTime() -_now;
                }
            });
            this.setData({
