...
Page({
    ...
    bindscroll(e) {
        ...
        const tabsHeight = this.data.tabsHeight; // 顶部距离（tabs高度）
        if (this.data.tabs[0].topHeight-tabsHeight<=0 && this.data.tabs[1].topHeight-tabsHeight > 0) { // 临界值，根据自己的需求来调整
            this.setData({
    ...
    async getGoodsDetailAndKanjieInfo(goodsId) {
        ...
        const that = this;
        const goodsDetailRes = await WXAPI.goodsDetail(goodsId, token || "");
   ...
   */
            ...
            p.childsCurGoods.forEach(c => {
                ...
                if (c.active) {
                    _skuList = _skuList.filter(aaa => {
                        return aaa.propertyChildIds.includes(`${p.id  }:${  c.id}`);
                    });
                    curSelectNum++;
    ...
    // 计算最终的商品价格
        ...
        const buyNumber = this.data.goodsDetail.basicInfo.minBuyNumber;
   ...
   */
    async buyNow(e) {
        ...
        const that = this;
        const shoptype = e.currentTarget.dataset.shoptype;
        if (this.data.goodsDetail.properties && !this.data.canSubmit) {
            wx.showToast({
                title: "请选择规格",
                icon: "none"
        ...
        // 组建立即购买信息
        const buyNowInfo = this.buliduBuyNowInfo(shoptype);
        // 写入本地存储
        wx.setStorage({
            key: "buyNowInfo",
            data: buyNowInfo
        ...
        if (shoptype === "toPingtuan") {
            if (this.data.pingtuanopenid) {
                ...
                WXAPI.pingtuanOpen(wx.getStorageSync("token"), that.data.goodsDetail.basicInfo.id).then((res) => {
                    if (res.code === 2000) {
                        return;
                    }
                    if (res.code !== 0) {
   ...
   */
    buliduBuyNowInfo (shoptype) {
        const shopCarMap = { goodsId: this.data.goodsDetail.basicInfo.id,  shopId: this.data.goodsDetail.basicInfo.shopId,  pic: this.data.goodsDetail.basicInfo.pic,  name: this.data.goodsDetail.basicInfo.name,  propertyChildIds: this.data.propertyChildIds,  label: this.data.propertyChildNames,  price: this.data.selectSizePrice,  score: this.data.totalScoreToPay,  left: "",  active: true,  number: this.data.buyNumber,  logisticsType: this.data.goodsDetail.basicInfo.logisticsId,  logistics: this.data.goodsDetail.logistics,  weight: this.data.goodsDetail.basicInfo.weight, };
        ...
        const buyNowInfo = { shopNum: 0,  shopList: [], };

        buyNowInfo.shopList.push(shopCarMap);
        buyNowInfo.kjId = this.data.kjId;
        buyNowInfo.shopInfo = this.data.shopSubdetail ? this.data.shopSubdetail.info : {
            id: 0,
            name: "其他",
            pic: null,
            serviceDistance: 99_999_999,
        };
        ...
        WXAPI.goodsReputationV2({
            ...
            if (res.code === 0) {
                res.data.result.forEach(ele => {
                    switch (ele.goods.goodReputation) {
                        case 0: {
    ...
    doneJoinKanjia () { // 报名参加砍价活动
        ...
        WXAPI.kanjiaJoin(wx.getStorageSync("token"), _this.data.curGoodsKanjia.id).then((res) => {
            wx.hideLoading();
            if (res.code === 0) {
                _this.setData({
    ...
    joinPingtuan (e) {
        const pingtuanopenid = e.currentTarget.dataset.pingtuanopenid;
        wx.navigateTo({
    ...
    helpKanjiaDone() {
        const _this = this;
        WXAPI.kanjiaHelp(wx.getStorageSync("token"), _this.data.kjId, _this.data.kjJoinUid, "").then((res) => {
            if (res.code !== 0) {
                wx.showToast({
                    title: res.msg,
                    icon: "none"
    ...
    savePosterPic() {
        ...
        wx.saveImageToPhotosAlbum({
            ...
            fail: (res) => {
                ...
                } else if (res.errMsg.includes("fail cancel")) {
                    wx.showToast({
                        title: "已取消",
                        icon: "none"
                    });
