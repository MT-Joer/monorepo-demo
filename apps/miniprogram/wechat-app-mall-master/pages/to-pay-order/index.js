...
Page({
    ...
    async doneShow() {
        ...
        if (this.data.orderType === "buyNow") {
            const buyNowInfoMem = wx.getStorageSync("buyNowInfo");
            ...
            if (this.data.shopCarType === 0) {// 自营购物车
                ...
                const res = await WXAPI.jdvopCartInfoV2(token);
                shopList = [ {
                    id: 0,
                    name: "其他",
                    hasNoCoupons: true,
    ...
    // shopCarType: 0 //0自营购物车，1云货架购物车
        ...
        if (e) {
            ...
            if (postData.peisongType === "zq") {
                ...
                postData.isCanHx = this.data.zt_open_hx === "1";
            }
        ...
        if (shopList && shopList.length > 1) {
            ...
            for (let index = 0; index < shopList.length; index++) {
                ...
                postData.couponId = curShop.curCoupon ? curShop.curCoupon.id : "";
                ...
                if (!e) {
                    ...
                    if (res.data.couponUserList) {
                        ...
                        res.data.couponUserList.forEach(ele => {
                            ...
                            ele.nameExt = ele.moneyHreshold ? `${ele.name  } [面值${  ele.money  }${moneyUnit  }，满${  ele.moneyHreshold  }元可用]` : `${ele.name  } [面值${  ele.money  }${moneyUnit  }]`;
                        });
                    ...
                    if (scoreDeductionRules) {
                        ...
                        scoreDeductionRules.forEach(ele => {
                            if (ele.loop) {
                                ...
                                const loopTimesMax = Math.floor((res.data.amountTotle + res.data.deductionMoney) / ele.money); // 按金额取最大
            ...
            // 单门店单商品下单
            if (shopList && shopList.length === 1) {
                postData.couponId = shopList[0].curCoupon ? shopList[0].curCoupon.id : "";
            }
            const res = await WXAPI.orderCreate(postData);
            this.data.pageIsEnd = true;
            if (res.code !== 0) {
        ...
        if (e && this.data.orderType !== "buyNow") {
            // 清空购物车数据
            const keyArrays = [];
    ...
    async processAfterCreateOrder(res) {
        ...
        let orderId = "";
        orderId = res.data.orderIds && res.data.orderIds.length > 0 ? res.data.orderIds.join(",") : res.data.id;
        // 直接弹出支付，取消支付的话，去订单列表
    ...
    processYunfei() {
        ...
        let inviter_id = 0;
        const inviter_id_storge = wx.getStorageSync("referrer");
        if (inviter_id_storge) {
            inviter_id = inviter_id_storge;
        }
        for (const carShopBean of goodsList) {
