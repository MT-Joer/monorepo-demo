...
Page({
    ...
    async delItemDone(key) {
        ...
        if (this.data.shopCarType === 1){
            const res = await WXAPI.jdvopCartRemoveV2(token, key);
        }
    ...
    async jiaBtnTap(e) {
        ...
        if (this.data.shopCarType === 0){
            ...
            const res = await WXAPI.jdvopCartModifyNumberV2(token, item.key, number);
        }
        this.shippingCarInfo();
    },
    async jianBtnTap(e) {
    ...
    async radioClick(e) {
        ...
        if (this.data.shopCarType === 0) { // 自营购物车
            ...
            const res = await WXAPI.jdvopCartSelectV2(token, item.key, !item.selected);
        }
        this.shippingCarInfo();
    },
    onChange(event) {
