...
async function smart(event) {
    ...
    let obj = {};
    let copyaddress = structuredClone(JSON.stringify(event));
    console.log("copyaddress", copyaddress);
    copyaddress = copyaddress.split(" ");
    console.log("地址转为数组:", copyaddress);
    for (let res of copyaddress) {
        if (res) {
            if (res.length === 1) {
                res += "XX"; // 过滤掉一位的名字或者地址
            }
            const addressObj = await smatrAddress(res);
...
async function smatrAddress(event) {
    ...
    //  if (endIndex > begIndex) {
        ...
        provinceList.forEach(res => {
            if (res.name.includes(matchAddress)) {
                matchProvince.push({
                    province: res.name,
                    provinceCode: res.id,
                            ...
                            element.children.forEach(item => {
                                if (item.name === smartObj.county) {
                                    item.children.forEach(res => {
                                        if (res.street.includes(matchAddress)) {
                                            matchStreet.push({
                                                street: res.street,
                                                streetCode: res.code,
                                                matchValue: matchAddress
...
function stripscript(s) {
    s = s.replaceAll("\t", " ");
    s = s.replaceAll(/(\d{3})-(\d{4})-(\d{4})/g, "$1$2$3");
    s = s.replaceAll(/(\d{3}) (\d{4}) (\d{4})/g, "$1$2$3");
    const pattern = new RegExp();
    let rs = "";
    for (let i = 0; i < s.length; i++) {
        rs = rs + s.slice(i, 1).replace(pattern, " ");
    }
    rs = rs.replaceAll(/[\r\n]/g, "");
    return rs;
}
