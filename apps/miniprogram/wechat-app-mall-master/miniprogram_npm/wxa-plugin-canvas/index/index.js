        ...
        if (textWidth > width) {
            ...
            for (let i = 0; i <= text.length - 1 ; i++) {  // 将文字转为数组，一行文字一个元素
                ...
                if (this.toRpx(this.ctx.measureText(fillText).width) >= width) {
                    if (line === lineNum && i !== text.length - 1) {
...
const helper = {
            ...
            this._downImage(imageUrl, index)
                ...
                .then(({ imgPath, imgInfo }) => {
                    ...
                    this.drawArr.push({
                        ...
                        zIndex: zIndex === undefined ? index : zIndex,
                        imgPath,
                        sx,
                        sy,
                        sw: (width - (sx * 2)),
    ...
    _downImage(imageUrl) {
        return new Promise((resolve, reject) => {
            if (imageUrl.startsWith("http") && !new RegExp(wx.env.USER_DATA_PATH).test(imageUrl)) {
                wx.downloadFile({
    ...
    toPx(rpx, int) {
        if (int) {
            return Number.parseInt(rpx * this.factor * this.pixelRatio);
        }
    ...
    toRpx(px, int) {
        if (int) {
            return Number.parseInt(px / this.factor);
        }
    ...
    methods: Object.assign({
        ...
        getHeight(config) {
            ...
            (config.lines || []).forEach((item) => {
                heightArr.push(item.startY, item.endY);
            });
            ...
            return config.height < canvasHeight || !config.height ? canvasHeight : config.height;
        },
        create(config) {
