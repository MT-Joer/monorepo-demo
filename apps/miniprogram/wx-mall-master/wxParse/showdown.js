...
const parsers = {};
const showdown = { helper: {},  extensions: {},  setOption (key, value) {
    "use strict";
    ...
    if (showdown.helper.isString(name)) {
        if (func === undefined) {
            if (parsers.hasOwnProperty(name)) {
                ...
                throw new Error(`SubParser named ${  name  } not registered!`);
            }
        } else {
            parsers[name] = func;
        }
...
showdown.extension = function (name, ext) {
    ...
    if (showdown.helper.isUndefined(ext)) {
        if (!extensions.hasOwnProperty(name)) {
            throw new Error(`Extension named ${  name  } is not registered!`);
        }
    ...
    // Expand extension if it's wrapped in a function
        ...
        if (validExtension.valid) {
            ...
            throw new Error(validExtension.error);
        }
    }
};
...
function validate(extension, name) {
    ...
    for (const [ i, ext ] of extension.entries()) {
        ...
        if (ext.listeners) {
            ...
            for (const ln in ext.listeners) {
                if (ext.listeners.hasOwnProperty(ln) && typeof ext.listeners[ln] !== "function") {
                    ret.valid = false;
...
showdown.helper.stdExtName = function (s) {
    "use strict";
    return s.replaceAll(/[_-]||\s/g, "").toLowerCase();
};
...
showdown.helper.escapeCharacters = function escapeCharacters(text, charsToEscape, afterBackslash) {
    ...
    let regexString = `([${  charsToEscape.replaceAll(/([[\]\\])/g, String.raw`\$1`)  }])`;
...
const rgxFindMatchPos = function (str, left, right, flags) {
    ...
    const l = new RegExp(left, f.replaceAll("g", ""));
    let m;
    const pos = []; let s; let start; let t; const x = new RegExp(`${left  }|${  right}`, `g${  f.replaceAll("g", "")}`);
...
showdown.helper.matchRecursiveRegExp = function (str, left, right, flags) {
    ...
    for (const matchPo of matchPos) {
        results.push([
            str.slice(matchPo.wholeMatch.start, matchPo.wholeMatch.end),
            str.slice(matchPo.match.start, matchPo.match.end),
            str.slice(matchPo.left.start, matchPo.left.end),
            str.slice(matchPo.right.start, matchPo.right.end)
        ]);
    }
    return results;
};
...
showdown.Converter = function (converterOptions) {
   ...
   */
    function _constructor() {
        ...
        if (typeof converterOptions === "object") {
            ...
            throw new TypeError(`Converter expects the passed parameter to be an object, but ${  typeof converterOptions
            } was passed instead.`);
        }
    ...
    function _parseExtension(ext, name) {
        ...
        if (showdown.helper.isString(ext)) {
          ...
          `Please inform the developer that the extension should be updated!`);
                ...
                throw new TypeError(`Extension "${  ext  }" could not be loaded. It was either not found or is not a valid extension.`);
            } else {
                ext = extensions[ext];
        ...
        if (!validExt.valid) {
            throw new Error(validExt.error);
        }
        ...
        for (const element of ext) {
            switch (element.type) {
                ...
                case "lang": {
                    langExtensions.push(element);
                    break;
                }

                case "output": {
                    outputModifiers.push(element);
                    break;
                }
            }
            if (element.hasOwnProperty(listeners)) {
                for (const ln in element.listeners) {
                    if (element.listeners.hasOwnProperty(ln)) {
                        listen(ln, element.listeners[ln]);
                    }
                }
            }
        }
   ...
   */
    function listen(name, callback) {
        if (!showdown.helper.isString(name)) {
            throw new TypeError(`Invalid argument in converter.listen() method: name must be a string, but ${  typeof name  } given`);
        }

        if (typeof callback !== "function") {
            throw new TypeError(`Invalid argument in converter.listen() method: callback must be a function, but ${  typeof callback  } given`);
        }
    ...
    function rTrimInputText(text) {
        ...
        const rgx = new RegExp(`${String.raw`^\s{0,` + rsp  }}`, "gm");
...
showdown.subParser("anchors", (text, options, globals) => {
    ...
    const writeAnchorTag = function (wholeMatch, m1, m2, m3, m4, m5, m6, m7) {
        ...
        if (!url) {
            if (!linkId) {
                ...
                linkId = linkText.toLowerCase().replaceAll(/ ?\n/g, " ");
            }
            url = `#${  linkId}`;
   ...
   */
   ...
   /g, writeAnchorTag);
   */
    text = text.replaceAll(/(\[([^[\]]+)\])()()()()()/g, writeAnchorTag);
...
showdown.subParser("autoLinks", (text, options, globals) => {
    ...
    const simpleURLRegex  = /\b(((https?|ftp|dict):\/\/|www\.)[^\s"'>][^\s"'.>]*\.[^\s"'>]+)(?=\s|$)(?!["<>])/gi;

    text = text.replaceAll(delimUrlRegex, replaceLink);
    text = text.replaceAll(delimMailRegex, replaceMail);
    // simpleURLRegex  = /\b(((https?|ftp|dict):\/\/|www\.)[-.+~:?#@!$&'()*,;=[\]\w]+)\b/gi,
    // Email addresses: <address@domain.foo>

    if (options.simplifiedAutoLink) {
        text = text.replaceAll(simpleURLRegex, replaceLink);
        text = text.replaceAll(simpleMailRegex, replaceMail);
    }
...
showdown.subParser("blockGamut", (text, options, globals) => {
    ...
    text = text.replaceAll(/^ {0,2}( ?\* ?){3,}[ \t]*$/gm, key);
    text = text.replaceAll(/^ {0,2}( ?- ?){3,}[ \t]*$/gm, key);
    text = text.replaceAll(/^ {0,2}( ?_ ?){3,}[ \t]*$/gm, key);
...
showdown.subParser("blockQuotes", (text, options, globals) => {
   ...
   */
    ...
    text = text.replaceAll(/((^[ \t]{0,3}>.+\n(.+\n)*\n*)+)/gm, (wholeMatch, m1) => {
        ...
        // "----------bug".replace(/^-/g,"") === "bug"
        bq = bq.replaceAll(/^[ \t]*>[ \t]?/gm, "~0"); // trim one level of quoting

        // attacklab: clean up hack
        bq = bq.replaceAll("~0", "");

        bq = bq.replaceAll(/^[ \t]+$/gm, ""); // trim whitespace-only lines
        bq = showdown.subParser("githubCodeBlocks")(bq, options, globals);
        bq = showdown.subParser("blockGamut")(bq, options, globals); // recurse

        bq = bq.replaceAll(/(^|\n)/g, "$1  ");
        // These leading spaces screw with <pre> content, so we need to fix that:
        bq = bq.replaceAll(/(\s*<pre>[^\r]+?<\/pre>)/g, (wholeMatch, m1) => {
            let pre = m1;
            // attacklab: hack around Konqueror 3.5.4 bug:
            pre = pre.replaceAll(/^ {2}/gm, "~0");
            pre = pre.replaceAll("~0", "");
            return pre;
        });
...
showdown.subParser("codeBlocks", (text, options, globals) => {
   ...
   */
    ...
    const pattern = /(?:\n\n|^)((?:(?: {4}|\t).*\n+)+)(\n* {0,3}[^ \t\n]|(?=~0))/g;
    text = text.replaceAll(pattern, (wholeMatch, m1, m2) => {
        let codeblock = m1;
        const nextChar = m2;
        let end = "\n";
...
showdown.subParser("codeSpans", (text, options, globals) => {
   ...
   */
    ...
    text = text.replaceAll(/(^|[^\\])(`+)([^\r]*?[^`])\2(?!`)/gm,
        (wholeMatch, m1, m2, m3) => {
            let c = m3;
            c = c.replaceAll(/^([ \t]*)/g, "");	// leading whitespace
            c = c.replaceAll(/[ \t]*$/g, "");	// trailing whitespace
...
showdown.subParser("detab", (text) => {
    ...
    text = text.replaceAll(/\t(?=\t)/g, "    "); // g_tab_width

    // replace the nth with two sentinels
    text = text.replaceAll("\t", "~A~B");

    // use the sentinel to anchor our regex so it doesn't explode
    text = text.replaceAll(/~B(.+?)~A/g, (wholeMatch, m1) => {
        let leadingText = m1;
    ...
    // clean up sentinels
    text = text.replaceAll("~A", "    ");  // g_tab_width
...
showdown.subParser("encodeAmpsAndAngles", (text) => {
    ...
    text = text.replaceAll(/&(?!#?x?(?:[0-9a-f]+|\w+);)/gi, "&amp;");
...
showdown.subParser("encodeBackslashEscapes", (text) => {
    "use strict";
    text = text.replaceAll(/\\(\\)/g, showdown.helper.escapeCharactersCallback);
    text = text.replaceAll(/\\([`*_{}[\]()>#+-.!])/g, showdown.helper.escapeCharactersCallback);
    return text;
});
...
showdown.subParser("encodeEmailAddress", (addr) => {
    ...
    addr = addr.replaceAll(/">.+:/g, '">'); // strip the mailto: from the visible part
...
showdown.subParser("escapeSpecialCharsWithinTagAttributes", (text) => {
    ...
    text = text.replaceAll(regex, (wholeMatch) => {
        let tag = wholeMatch.replaceAll(/(.)<\/?code>(?=.)/g, "$1`");
...
showdown.subParser("hashHTMLBlocks", (text, options, globals) => {
    ...
    for (const blockTag of blockTags) {
        text = showdown.helper.replaceRecursiveRegExp(text, repFunc, String.raw`^(?: |\t){0,3}<` + blockTag + String.raw`\b[^>]*>`, `</${  blockTag  }>`, "gim");
    }
...
showdown.subParser("hashHTMLSpans", (text, config, globals) => {
    ...
    const matches = showdown.helper.matchRecursiveRegExp(text, String.raw`<code\b[^>]*>`, "</code>", "gi");

    for (const match of matches) {
        text = text.replace(match[0], `~L${  globals.gHtmlSpans.push(match[0]) - 1  }L`);
    }
    return text;
});
...
showdown.subParser("hashPreCodeTags", (text, config, globals) => {
    ...
    text = showdown.helper.replaceRecursiveRegExp(text, repFunc, String.raw`^(?: |\t){0,3}<pre\b[^>]*>\s*<code\b[^>]*>`, String.raw`^(?: |\t){0,3}</code>\s*</pre>`, "gim");
    return text;
});
...
showdown.subParser("headers", (text, options, globals) => {
    ...
    const headerLevelStart = (isNaN(Number.parseInt(options.headerLevelStart))) ? 1 : Number.parseInt(options.headerLevelStart);
    ...
    function headerId(m) {
        const escapedId = m.replaceAll(/\W/g, "").toLowerCase(); let title;
...
showdown.subParser("images", (text, options, globals) => {
    ...
    function writeImageTag (wholeMatch, altText, linkId, url, width, height, m5, title) {
        ...
        if (url === "" || url === null) {
            if (linkId === "" || linkId === null) {
                // lower-case and turn embedded newlines into spaces
                linkId = altText.toLowerCase().replaceAll(/ ?\n/g, " ");
            }
            url = `#${  linkId}`;
    ...
    text = text.replaceAll(referenceRegExp, writeImageTag);

    // Next, handle inline images:  ![alt text](url =<width>x<height> "optional title")
    text = text.replaceAll(inlineRegExp, writeImageTag);
...
showdown.subParser("italicsAndBold", (text, options, globals) => {
    ...
    // Since we are consuming a \s character, we need to add it
        text = text.replaceAll(/(^|\s|>|\b)__(?=\S)([\s\S]+?)__(?=\b|<|\s|$)/gm, "$1<strong>$2</strong>");
        text = text.replaceAll(/(^|\s|>|\b)_(?=\S)([\s\S]+?)_(?=\b|<|\s|$)/gm, "$1<em>$2</em>");
        // asterisks
        text = text.replaceAll(/(\*\*)(?=\S)([^\r]*?\S\**)\1/g, "<strong>$2</strong>");
        text = text.replaceAll(/(\*)(?=\S)([^\r]*?\S)\1/g, "<em>$2</em>");

    } else {
    // <strong> must go first:
        text = text.replaceAll(/(\*\*|__)(?=\S)([^\r]*?\S[*_]*)\1/g, "<strong>$2</strong>");
        text = text.replaceAll(/(\*|_)(?=\S)([^\r]*?\S)\1/g, "<em>$2</em>");
    }
...
showdown.subParser("lists", (text, options, globals) => {
   ...
   */
    ...
    // starting cardinal number; e.g. "1." or "a.".
        ...
        listStr = listStr.replaceAll(rgx, (wholeMatch, m1, m2, m3, m4, taskbtn, checked) => {
            ...
            if (m1 || (item.search(/\n{2,}/) > -1)) {
                ...
                item = item.replace(/\n$/, ""); // chomp(item)
                item = isParagraphed ? showdown.subParser("paragraphs")(item, options, globals) : showdown.subParser("spanGamut")(item, options, globals);
            }
...
showdown.subParser("outdent", (text) => {
    ...
    // attacklab: hack around Konqueror 3.5.4 bug:
    // "----------bug".replace(/^-/g,"") === "bug"
    text = text.replaceAll(/^(\t| {1,4})/gm, "~0"); // attacklab: g_tab_width
...
showdown.subParser("paragraphs", (text, options, globals) => {
    ...
    for (i = 0; i < end; i++) {
        ...
        while (grafsOutIt.search(/~(K|G)(\d+)\1/) >= 0) {
            ...
            blockText = blockText.replaceAll("$", "$$$$"); // Escape any dollar signs
...
showdown.subParser("strikethrough", (text, options, globals) => {
    ...
    if (options.strikethrough) {
        ...
        text = text.replaceAll(/(?:~T){2}([\s\S]+?)(?:~T){2}/g, "<del>$1</del>");
 ...
 * contorted like /[ \t]*\n+/
 */
showdown.subParser("stripBlankLines", (text) => {
    "use strict";
...
showdown.subParser("stripLinkDefinitions", (text, options, globals) => {
    ...
    text = text.replaceAll(regex, (wholeMatch, linkId, url, width, height, blankLines, title) => {
        linkId = linkId.toLowerCase();
        ...
        if (blankLines) {
            ...
            if (title) {
                globals.gTitles[linkId] = title.replaceAll(/"|'/g, "&quot;");
            }
...
showdown.subParser("tables", (text, options, globals) => {
    ...
    function parseHeaders(header, style) {
        ...
        if (options.tableHeaderId) {
            id = ` id="${  header.replaceAll(" ", "_").toLowerCase()  }"`;
        }
    ...
    text = text.replaceAll(tableRgx, (rawTable) => {

        let i; const tableLines = rawTable.split("\n");
        ...
        const headers = [];
        const rawCells = [];
        const rawHeaders = tableLines[0].split("|").map((s) => { return s.trim();});
        const rawStyles = tableLines[1].split("|").map((s) => { return s.trim();});
        const styles = [];
...
showdown.subParser("unescapeSpecialChars", (text) => {
    ...
    text = text.replaceAll(/~E(\d+)E/g, (wholeMatch, m1) => {
