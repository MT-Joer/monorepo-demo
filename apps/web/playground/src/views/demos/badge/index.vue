<template>
    <Page description="菜单项上可以显示徽标，这些徽标可以主动更新"
          title="菜单徽标">
        <Card title="徽标更新">
            <Form>
                <template #badgeVariants="slotProps">
                    <RadioGroup v-bind="slotProps">
                        <Radio v-for="color in colors"
                               :key="color.value"
                               :value="color.value">
                            <div class="flex h-[14px] w-[50px] items-center justify-start"
                                 :title="color.label">
                ...
                <template #action>
                    <Button type="primary" @click="updateMenuBadge">
                        更新徽标
                    </Button>
                </template>
            </Form>
