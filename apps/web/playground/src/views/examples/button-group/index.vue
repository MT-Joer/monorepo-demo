<template>
    <Page description="VbenButtonGroup是一个按钮容器，用于包裹一组按钮，协调整体样式。VbenCheckButtonGroup则可以作为一个表单组件，提供单选或多选功能"
          title="VbenButtonGroup 按钮组">
        <Card title="基本用法">
            <template #extra>
                <Button type="primary" @click="resetValues">
                    清空值
                </Button>
            </template>
            <p class="mt-4">
            ...
            <div class="mt-2 flex flex-col gap-2">
                <VbenButtonGroup v-bind="compProps" border>
                    <VbenButton v-for="btn in options"
                                :key="btn.value"
                                variant="link"
            ...
            <div class="mt-2 flex flex-col gap-2">
                <VbenCheckButtonGroup v-model="radioValue"
                                      :options="options"
                                      v-bind="compProps" />
            </div>
            <p class="mt-4">
            ...
            <div class="mt-2 flex flex-col gap-2">
                <VbenCheckButtonGroup v-model="checkValue"
                                      multiple
                                      :options="options"
                                      v-bind="compProps" />
            </div>
